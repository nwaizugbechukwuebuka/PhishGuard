...
class SIEMExporter:
    ...
    def format_threat_event_cef(self, email: Email, threat_details: Dict[str, Any]) -> str:
        """
        ...
        try:
            ...
            severity = self._map_threat_level_to_cef_severity(threat_details.get("threat_level", "low"))
    ...
    def format_threat_event_leef(self, email: Email, threat_details: Dict[str, Any]) -> str:
        """
        ...
        try:
            ...
            attributes.append(f"sev={self._map_threat_level_to_leef_severity(threat_details.get('threat_level', 'low'))}")
            attributes.append(f"msg={email.subject or 'No Subject'}")
            
            # Custom attributes
            risk_score = threat_details.get("risk_score", 0.0)
            attributes.append(f"riskScore={risk_score:.3f}")
            attributes.append(f"threatLevel={threat_details.get('threat_level', 'low')}")
    ...
    def format_threat_event_json(self, email: Email, threat_details: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def format_threat_event_syslog(self, email: Email, threat_details: Dict[str, Any]) -> str:
        """
    ...
    async def export_threat_event(self, email: Email, threat_details: Dict[str, Any]) -> bool:
        """
                ...
                if self.format_type == SIEMFormat.JSON:
                    async with session.post(
                        ...
                        if response.status in [200, 201, 202]:
                            ...
                            logger.error(f"SIEM export failed: {response.status} - {error_text}")
                            return False
                else:
                    async with session.post(
                        self.siem_url,
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                async with session.post(
                    ...
                    if response.status in [200, 201, 202]:
                        ...
                        error_text = await response.text()
                        logger.error(f"SIEM audit export failed: {response.status} - {error_text}")
                        return False
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                async with session.post(
                    ...
                    if response.status in [200, 201, 202]:
                        ...
                        error_text = await response.text()
                        logger.error(f"SIEM batch export failed: {response.status} - {error_text}")
                        return False
...
async def create_siem_exporter(format_type: SIEMFormat = SIEMFormat.JSON) -> SIEMExporter:
