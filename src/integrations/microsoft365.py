...
class Microsoft365Integration:
    ...
    def __init__(
        ...
        self.redirect_uri = redirect_uri or os.getenv('MICROSOFT365_REDIRECT_URI', 'http://localhost:8000/auth/microsoft365/callback')
    ...
    async def authenticate_with_code(self, authorization_code: str) -> bool:
        ...
        try:
            ...
            if "access_token" in result:
                ...
                logger.error(f"Microsoft 365 authentication failed: {result.get('error_description')}")
                return False
    ...
    async def authenticate_with_client_credentials(self) -> bool:
        ...
        try:
            ...
            result = self.msal_client.acquire_token_for_client(scopes=["https://graph.microsoft.com/.default"])
            
            if "access_token" in result:
                self.access_token = result["access_token"]
                self.token_expires_at = datetime.now() + timedelta(seconds=result.get("expires_in", 3600))
                
                logger.info("Microsoft 365 client credentials authentication successful")
                return True
            else:
                logger.error(f"Microsoft 365 client credentials authentication failed: {result.get('error_description')}")
                return False
                ...
                if method.upper() == "GET":
                    async with session.get(url, headers=headers, params=params) as response:
                        if response.status == 200:
                            ...
                            raise Microsoft365Error(f"Graph API error: {response.status} - {error_text}")
    ...
    def parse_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            cc_emails = [r.get("emailAddress", {}).get("address", "") for r in cc_recipients]
            
            bcc_recipients = message.get("bccRecipients", [])
            bcc_emails = [r.get("emailAddress", {}).get("address", "") for r in bcc_recipients]
            ...
            for message_meta in messages:
                try:
                    ...
                    email = Email(
                        ...
                        body_text=parsed["body_content"] if parsed["body_type"] == "text" else "",
                        body_html=parsed["body_content"] if parsed["body_type"] == "html" else "",
                        received_date=datetime.fromisoformat(parsed["received_date"].replace("Z", "+00:00")) if parsed["received_date"] else datetime.now(),
                        user_id=user.id,
                    ...
                    logger.error(f"Error processing Microsoft 365 message {message_meta.get('id')}: {e}")
                    continue
    ...
    async def quarantine_message(self, message_id: str) -> bool:
        ...
        try:
            ...
            quarantine_folder_id = await self.get_or_create_folder("PhishGuard-Quarantine")
            
            # Move message to quarantine folder
            move_data = {
                "destinationId": quarantine_folder_id
            }
    ...
    async def get_or_create_folder(self, folder_name: str) -> str:
        ...
        try:
            ...
            created_folder = await self.make_graph_request(endpoint, method="POST", data=folder_data)
...
async def scan_microsoft365_for_threats(user: User, integration: Microsoft365Integration = None) -> List[Email]:
