...
class SlackIntegration:
    ...
    def __init__(
        self, webhook_url: str = None, channel: str = None, username: str = None
    ):
        """
        ...
        self.default_channel = channel or os.getenv(
            "SLACK_CHANNEL", "#phishguard-alerts"
        )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200:
                        logger.info(
                            f"Slack message sent successfully to {channel or self.default_channel}"
                        )
                        return True
                    else:
    ...
    async def send_threat_alert(
        self, email: Email, threat_details: Dict[str, Any], channel: str = None
    ) -> bool:
        """
        ...
        try:
            ...
            attachment = {
                ...
                "fields": [
                    {"title": "Sender", "value": email.sender_email, "short": True},
                    {
                        "title": "Recipient",
                    ...
                    },
                    {
                        "title": "Status",
                        "value": (
                            "Quarantined" if email.is_quarantined else "Under Review"
                        ),
                        "short": True,
                    },
                ],
            ...
            if indicators:
                indicator_text = "\n".join(
                    [f"• {indicator}" for indicator in indicators[:5]]
                )
        ...
        try:
            ...
            attachment = {
                ...
                "fields": [
                    ...
                    {"title": "Action Taken", "value": action.title(), "short": True},
                    {
                        "title": "Timestamp",
            ...
            if user_email:
                attachment["fields"].append(
                    {"title": "Performed By", "value": user_email, "short": True}
                )

            alert_text = f"📧 Email {action} successfully"

            return await self.send_message(
                text=alert_text, channel=channel, attachments=[attachment]
            )
    ...
    async def send_daily_summary(
        self, summary_data: Dict[str, Any], channel: str = None
    ) -> bool:
        """
        ...
        try:
            ...
            if top_threats:
                ...
                attachment["fields"].append(
                    {"title": "Top Threat Types", "value": threat_text, "short": False}
                )
    ...
    async def send_user_report_notification(
        ...
        try:
            attachment = {
                ...
                "fields": [
                    {"title": "Reported By", "value": reporter_email, "short": True},
                    {
                        "title": "Report Type",
...
async def send_system_alert_to_slack(
