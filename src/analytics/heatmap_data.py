...
class HeatmapDataGenerator:
        ...
        try:
            ...
            return {
                ...
                "metadata": {
                    ...
                    "total_threats": sum(
                        data["count"] for data in geographic_threats.values()
                    ),
                },
                "insights": await self._generate_geographic_insights(
                    geographic_threats
                ),
            }
    ...
    async def generate_department_heatmap(
        ...
        try:
            ...
            for dept in department_analysis:
                ...
                normalized = (
                    data["threat_rate"] / max_threat_rate if max_threat_rate > 0 else 0
                )
            ...
            return {
                ...
                "insights": await self._generate_department_insights(
                    department_analysis
                ),
            }
    ...
    async def generate_threat_type_heatmap(
        ...
        try:
            ...
            threat_type_data = (
                ...
                .group_by(Email.threat_type, "week")
                .all()
            )
            ...
            return {
                ...
                "insights": await self._generate_threat_type_insights(
                    normalized_matrix
                ),
            }
    ...
    async def generate_sender_reputation_heatmap(
        ...
        try:
            ...
            reputation_data = (
                ...
                .filter(
                    and_(Email.received_at >= start_date, Email.received_at <= end_date)
                )
                .group_by("reputation_range")
                .all()
            )
            ...
            for row in reputation_data:
                risk_normalized = (
                    (row.avg_risk_score or 0) / max_risk_score
                    if max_risk_score > 0
                    else 0
                )
                count_normalized = (
                    row.email_count / max_email_count if max_email_count > 0 else 0
                )
                ...
                heatmap_data[row.reputation_range] = {
                    ...
                    "combined_intensity": self._get_intensity_level(
                        (risk_normalized + count_normalized) / 2
                    ),
                }
    ...
    async def _generate_temporal_insights(
        ...
        # Find peak hours
        hourly_totals = [
            sum(heatmap_grid[day][hour] for day in range(7)) for hour in range(24)
        ]
    ...
    async def _generate_daily_insights(self, heatmap_data: List[Dict]) -> List[str]:
        ...
        if len(counts) >= 7:
            ...
            earlier_avg = (
                sum(counts[:-7]) / len(counts[:-7]) if len(counts) > 7 else recent_avg
            )
    ...
    async def _generate_geographic_insights(self, geographic_data: Dict) -> List[str]:
        ...
        sorted_countries = sorted(
            geographic_data.items(), key=lambda x: x[1]["count"], reverse=True
        )
    ...
    async def _generate_department_insights(self, department_data: Dict) -> List[str]:
        ...
        highest_risk = (
            max(department_data.items(), key=lambda x: x[1]["threat_rate"])
            if department_data
            else None
        )
        ...
        if highest_risk:
            ...
            insights.append(
                f"{dept_name} has highest threat rate: {data['threat_rate']:.2f} threats/user"
            )
        ...
        if high_risk_depts:
            insights.append(
                f"{len(high_risk_depts)} departments classified as high risk"
            )
    ...
    async def _generate_threat_type_insights(self, matrix_data: Dict) -> List[str]:
        ...
        if threat_totals:
            ...
            insights.append(
                f"Most prevalent threat type: {most_common[0]} ({most_common[1]} incidents)"
            )
    ...
    async def _generate_reputation_insights(self, reputation_data: Dict) -> List[str]:
        ...
        for reputation_range, data in reputation_data.items():
            if "Low" in reputation_range and data["avg_risk_score"] > 70:
                insights.append(
                    f"{reputation_range} senders have high average risk score: {data['avg_risk_score']}"
                )
        ...
        for reputation_range, data in reputation_data.items():
            percentage = (
                (data["email_count"] / total_emails) * 100 if total_emails > 0 else 0
            )
            if percentage > 30:
                insights.append(
                    f"{reputation_range} senders account for {percentage:.1f}% of emails"
                )
...
async def generate_temporal_heatmap(
    start_date: datetime, end_date: datetime, granularity: str = "hour"
) -> Dict[str, Any]:
    """Generate temporal threat heatmap."""
    db = next(get_db())
    try:
        generator = HeatmapDataGenerator(db)
        return await generator.generate_temporal_heatmap(
...
async def generate_all_heatmaps(
    ...
    try:
        ...
        reputation_task = generator.generate_sender_reputation_heatmap(
            start_date, end_date
        )

        temporal, geographic, department, threat_type, reputation = (
