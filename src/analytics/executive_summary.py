...
class ExecutiveSummaryGenerator:
    ...
    async def _generate_security_metrics(self, start_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            total_emails = (
                self.db.query(Email).filter(Email.received_at >= start_date).count()
            )

            # Threats detected and blocked
            threats_detected = (
                self.db.query(Email)
                .filter(and_(Email.received_at >= start_date, Email.is_threat == True))
                .count()
            )
            ...
            quarantined_emails = (
                ...
                .filter(QuarantinedEmail.quarantined_at >= start_date)
                .count()
            )

            # Average threat score
            avg_threat_score = (
                self.db.query(func.avg(Email.risk_score))
                .filter(and_(Email.received_at >= start_date, Email.is_threat == True))
                .scalar()
                or 0
            )
            ...
            threat_types = (
                self.db.query(Email.threat_type, func.count(Email.id))
                .filter(and_(Email.received_at >= start_date, Email.is_threat == True))
                .group_by(Email.threat_type)
                .all()
            )
            ...
            return {
                ...
                "attack_success_rate": (
                    ...
                    / max(
                        ...
                        .filter(
                            and_(
                                Email.received_at >= start_date, Email.is_threat == True
                            )
                        )
                        .count(),
                        1,
                ...
                * 100,
                "user_vulnerability_index": self._calculate_user_vulnerability_index(
                    high_risk_users
                ),
            }
    ...
    async def _generate_operational_metrics(
        ...
        try:
            ...
            quarantine_review_time = (
                self.db.query(
                    func.avg(
                        func.extract(
                            "epoch",
                            QuarantinedEmail.reviewed_at
                            - QuarantinedEmail.quarantined_at,
                        )
                    )
                )
                .filter(
            ...
            return {
                ...
                "automation_rate": (
                    auto_resolved / max(auto_resolved + manual_reviewed, 1)
                )
                * 100,
    ...
    async def _generate_compliance_metrics(
        ...
        try:
            ...
            audit_events = (
                self.db.query(AuditLog).filter(AuditLog.timestamp >= start_date).count()
            )
    ...
    async def _generate_financial_metrics(self, start_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            cost_avoidance = (
                threats_blocked * avg_incident_cost * 0.1
            )  # 10% would have been successful
            ...
            # ROI calculation
            investment = monthly_operational_cost * (
                (datetime.utcnow() - start_date).days / 30
            )
            ...
            return {
                ...
                "total_value_delivered_usd": round(
                    cost_avoidance + productivity_value, 2
                ),
    ...
    async def _generate_trend_analysis(self, start_date: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(weekly_trends) >= 2:
                ...
                earlier_avg = (
                    np.mean([w["threat_rate"] for w in weekly_trends[:-2]])
                    if len(weekly_trends) > 2
                    else recent_avg
                )
                trend_direction = (
    ...
    async def _generate_threat_landscape(self, start_date: datetime) -> Dict[str, Any]:
        ...
        try:
            # Top threat sources
            threat_sources = (
                self.db.query(Email.sender_domain, func.count(Email.id))
                .filter(and_(Email.received_at >= start_date, Email.is_threat == True))
    ...
    def _create_executive_summary(
        self, security_metrics: Dict, risk_metrics: Dict, operational_metrics: Dict
    ) -> Dict[str, Any]:
    ...
    def _calculate_organizational_risk_score(
        self, critical_threats: int, successful_attacks: int, high_risk_user_count: int
    ) -> int:
