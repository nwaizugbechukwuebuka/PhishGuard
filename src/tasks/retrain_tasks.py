...
def retrain_phishing_model(
    ...
    try:
        ...
        if new_emails_count < min_samples and not force_retrain:
            logger.info(
                f"Insufficient new data for retraining: {new_emails_count} < {min_samples}"
            )
            return {
                "status": "skipped",
        ...
        if len(training_data) < min_samples:
            logger.warning(
                f"Total training data insufficient: {len(training_data)} < {min_samples}"
            )
            return {
                "status": "failed",
        ...
        training_results = model_trainer.train_model(X_train, y_train, X_val, y_val)
        ...
        should_deploy = should_deploy_new_model(evaluation_results, comparison_results)
        ...
        if self.request.retries < self.max_retries:
            logger.info(
                f"Retrying model retraining (attempt {self.request.retries + 1})"
            )
...
def update_feature_importance() -> Dict[str, Any]:
    ...
    try:
        ...
        importance_file = os.path.join(
            settings.MODEL_STORAGE_PATH, "feature_importance.json"
        )
...
def validate_model_performance() -> Dict[str, Any]:
    ...
    try:
        ...
        if len(recent_emails) < 100:
            logger.warning(
                f"Insufficient validation data: {len(recent_emails)} samples"
            )
            return {
        ...
        for email in recent_emails:
            ...
            true_labels.append(
                email.feedback_label
                if email.feedback_label is not None
                else email.is_threat
            )
        ...
        false_positives = sum(
            1 for t, p in zip(true_labels, predictions) if not t and p
        )
        true_negatives = sum(
            1 for t, p in zip(true_labels, predictions) if not t and not p
        )
        false_negatives = sum(
            1 for t, p in zip(true_labels, predictions) if t and not p
        )
        ...
        validation_results = {
            ...
            "false_positive_rate": round(
                false_positives / max(1, false_positives + true_negatives), 4
            ),
            "false_negative_rate": round(
                false_negatives / max(1, false_negatives + true_positives), 4
            ),
        }
    ...
    try:
        ...
        if os.path.exists(metadata_file):
            ...
            return datetime.fromisoformat(
                metadata.get("last_training_date", "2023-01-01T00:00:00")
            )
...
def prepare_training_data(db: Session, since_date: datetime) -> List[Email]:
    ...
    emails = (
        db.query(Email)
        .filter(Email.processed_at >= since_date, Email.is_threat.isnot(None))
        .all()
    )

    # Include feedback-labeled emails
    feedback_emails = db.query(Email).filter(Email.feedback_label.isnot(None)).all()
...
def compare_model_performance(
    current_model_path: str, new_model, X_val: np.ndarray, y_val: np.ndarray
) -> Dict[str, Any]:
...
def should_deploy_new_model(
    evaluation_results: Dict[str, float], comparison_results: Dict[str, Any]
) -> bool:
    ...
    meets_requirements = (
        evaluation_results["accuracy"] >= min_accuracy
        and evaluation_results["precision"] >= min_precision
        and evaluation_results["recall"] >= min_recall
        and evaluation_results["f1_score"] >= min_f1
    )
...
def deploy_new_model(model) -> Dict[str, Any]:
    ...
    try:
        ...
        if os.path.exists(model_path):
            backup_path = (
                model_path + f'.backup_{datetime.utcnow().strftime("%Y%m%d_%H%M%S")}'
            )
...
def update_training_metadata(
    ...
    try:
        metadata_file = os.path.join(
            settings.MODEL_STORAGE_PATH, "training_history.json"
        )
...
def cleanup_old_backups(backup_dir: str, keep_count: int = 10) -> None:
    ...
    try:
        ...
        backup_dirs = [
            ...
            and d.startswith("model_backup_")
        ]
