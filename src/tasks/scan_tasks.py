...
def scan_email(self, email_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        logger.info(
            f"Starting email scan for message ID: {email_data.get('message_id')}"
        )
        ...
        if not message_id or not sender or not recipient:
            raise ValueError(
                "Missing required email fields: message_id, sender, or recipient"
            )
        ...
        ai_score = phishing_detector.predict_single_email(
            subject=subject, body=body, sender=sender, headers=headers
        )
        ...
        threat_score = max(
            ai_score.get("threat_score", 0), rule_results.get("threat_score", 0)
        )
        threat_type = ai_score.get("threat_type") or rule_results.get(
            "threat_type", "unknown"
        )
        confidence = min(
            ai_score.get("confidence", 0), rule_results.get("confidence", 0)
        )
        ...
        if is_threat:
            logger.warning(
                f"Threat detected in email {message_id}: {threat_type} (score: {threat_score})"
            )
        ...
        # Log scan completion
        logger.info(
            f"Email scan completed for {message_id}: threat={is_threat}, score={threat_score}"
        )
        ...
        if (
            ...
            logger.info(
                f"Retrying email scan for {email_data.get('message_id')} (attempt {self.request.retries + 1})"
            )
            raise self.retry(exc=exc, countdown=60 * (self.request.retries + 1))

        # Log final failure
        logger.error(
            f"Failed to scan email {email_data.get('message_id')} after {self.request.retries} retries"
        )
...
def scan_inbox_batch(
    self, provider: str, account_id: str, batch_size: int = 50
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(
            f"Batch scan completed: {len(results)} emails processed, {threats_detected} threats detected"
        )
...
def scan_all_inboxes() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"All inbox scan completed: {total_emails} emails, {total_threats} threats"
        )
...
def rescan_quarantined_email(self, quarantine_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        quarantine_item = (
            db.query(QuarantineItem).filter(QuarantineItem.id == quarantine_id).first()
        )
        ...
        if should_release:
            logger.info(
                f"Releasing email {email.id} after rescan (score: {new_threat_score})"
            )
            quarantine_service.release_email(
                quarantine_id, reason="Rescan determined not a threat"
            )

        db.commit()

        logger.info(
            f"Rescan completed for quarantine {quarantine_id}: {old_threat_score} -> {new_threat_score}"
        )
