...
def send_threat_notification(
    self, email_id: int, threat_type: str, threat_score: float
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        for recipient in recipients:
            if recipient.email_notifications:
                try:
                    ...
                    logger.error(
                        f"Failed to send email to {recipient.email}: {str(exc)}"
                    )
        ...
        if settings.SLACK_WEBHOOK_URL:
            try:
                slack_result = send_slack_notification.delay(
                    threat_details=threat_details, channel="#security-alerts"
                ).get(timeout=30)
            except Exception as exc:
                logger.error(f"Failed to send Slack notification: {str(exc)}")
                slack_result = {"status": "failed", "error": str(exc)}
        ...
        logger.error(
            f"Error sending threat notification for email {email_id}: {str(exc)}"
        )

        if self.request.retries < self.max_retries:
            logger.info(
                f"Retrying threat notification (attempt {self.request.retries + 1})"
            )
...
def send_email_notification(
    ...
    try:
        ...
        result = mail_client.send_email(
            to_email=recipient_email, subject=subject, body=body, is_html=True
        )
        ...
        if result["success"]:
            ...
            logger.error(
                f"Failed to send email to {recipient_email}: {result.get('error')}"
            )
            return {
                "status": "failed",
                "recipient": recipient_email,
        ...
        logger.error(
            f"Error sending email notification to {recipient_email}: {str(exc)}"
        )
...
def send_slack_notification(
    self, threat_details: Dict[str, Any], channel: str = "#security-alerts"
) -> Dict[str, Any]:
    """
            ...
            "attachments": [
                {
                    "color": (
                        "danger"
                        if threat_details.get("threat_score", 0) > 0.8
                        else "warning"
                    ),
                    ...
                    "fields": [
                        ...
                        },
                        {
                            ...
                            "value": (
                                "Quarantined"
                                if threat_details.get("quarantined")
                                else "Flagged"
                            ),
                            "short": True,
                        },
...
def send_daily_digest() -> Dict[str, Any]:
    ...
    try:
        ...
        total_emails = (
            db.query(Email)
            .filter(Email.processed_at.between(start_time, end_time))
            .count()
        )
        ...
        threat_types = (
            db.query(Email.threat_type, db.func.count(Email.id))
            .filter(
                Email.processed_at.between(start_time, end_time),
                Email.is_threat == True,
            )
        ...
        digest_data = {
            ...
            "detection_rate": (
                (threats_detected / total_emails * 100) if total_emails > 0 else 0
            ),
        }
...
def send_system_alert(
    ...
    try:
        ...
        if severity in ["high", "critical"]:
            ...
            recipients = (
                db.query(User)
                .filter(User.role.in_(["admin"]), User.system_alerts_enabled == True)
                .all()
            )
        ...
        for recipient in recipients:
            if recipient.email_notifications:
                try:
                    ...
                    logger.error(
                        f"Failed to send system alert to {recipient.email}: {str(exc)}"
                    )
                    email_results.append(
                        {
                            "status": "failed",
        ...
        if severity == "critical" and settings.SLACK_WEBHOOK_URL:
            try:
                ...
                response = requests.post(
                    settings.SLACK_WEBHOOK_URL, json=slack_payload, timeout=30
                )
