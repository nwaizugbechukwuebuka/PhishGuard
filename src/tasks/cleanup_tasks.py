...
def cleanup_old_emails(days_to_keep: int = 90) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Email cleanup completed: {emails_deleted} emails, {files_deleted} files, {space_freed} bytes freed"
        )
...
def cleanup_quarantine_files(days_to_keep: int = 365) -> Dict[str, Any]:
    ...
    try:
        logger.info(
            f"Starting cleanup of quarantine files older than {days_to_keep} days"
        )
        ...
        quarantine_storage_path = os.path.join(
            settings.QUARANTINE_STORAGE_PATH, "quarantined_emails"
        )
        ...
        logger.info(
            f"Quarantine cleanup completed: {items_processed} items, {files_deleted} files, {attachments_deleted} attachments"
        )
...
def cleanup_old_notifications(days_to_keep: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Notification cleanup completed: {notifications_count} notifications deleted"
        )
...
def cleanup_audit_logs(
    days_to_keep: int = 2555,
) -> Dict[str, Any]:  # ~7 years for compliance
    """
...
def cleanup_simulation_data(days_to_keep: int = 180) -> Dict[str, Any]:
    ...
    try:
        logger.info(
            f"Starting cleanup of simulation data older than {days_to_keep} days"
        )
        ...
        logger.info(
            f"Simulation data cleanup completed: {results_count} results deleted"
        )
...
def optimize_database() -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            db.execute(
                "UPDATE pg_stat_user_tables SET reltuples = (SELECT count(*) FROM emails) WHERE relname = 'emails';"
            )
...
def cleanup_temp_files() -> Dict[str, Any]:
    ...
    try:
                ...
                for root, dirs, files in os.walk(temp_dir):
                    ...
                    for file in files:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Could not delete temp file {file_path}: {str(exc)}"
                            )
                            continue
                    ...
                    for dir_name in dirs:
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Could not delete temp directory {dir_path}: {str(exc)}"
                            )
                            continue
        ...
        logger.info(
            f"Temp file cleanup completed: {files_deleted} files, {directories_deleted} directories"
        )
...
def generate_system_health_report() -> Dict[str, Any]:
    ...
    try:
        ...
        recent_threats = (
            db.query(Email)
            .filter(Email.processed_at >= yesterday, Email.is_threat == True)
            .count()
        )
        ...
        health_report = {
            ...
            "recent_activity": {
                ...
                "threat_rate_last_24h": round(
                    (recent_threats / recent_emails * 100) if recent_emails > 0 else 0,
                    2,
                ),
            },
            "system_performance": {
            ...
            "health_score": calculate_system_health_score(
                detection_rate, recent_emails, disk_space_result.get("disk_usage", {})
            ),
        }
...
def archive_simulation_data(simulation_results: List) -> Optional[str]:
    ...
    try:
        ...
        archive_filename = (
            f"simulation_data_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.csv"
        )
        archive_path = os.path.join(archive_dir, archive_filename)
                ...
                writer.writerow(
                    {
                        ...
                        "clicked_at": (
                            result.clicked_at.isoformat() if result.clicked_at else None
                        ),
                        "reported": result.reported,
                        "reported_at": (
                            result.reported_at.isoformat()
...
def calculate_system_health_score(
    detection_rate: float, recent_activity: int, disk_usage: Dict[str, Any]
) -> int:
