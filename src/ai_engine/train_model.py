...
class PhishingModelTrainer:
    ...
    async def _generate_synthetic_data(self) -> pd.DataFrame:
        ...
        for _ in range(1000):
            ...
            if np.random.random() > 0.5:
                sample['email_content'] = self._add_text_variation(sample['email_content'])
    ...
    def _preprocess_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"Dataset balance - Phishing: {phishing_count}, Legitimate: {legitimate_count}")
    ...
    async def hyperparameter_tuning(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        """
        ...
        logger.info(f"🔧 Performing hyperparameter tuning for {self.best_model_name}...")
        ...
        try:
            ...
            logger.info(f"✅ Hyperparameter tuning complete. Best F1: {grid_search.best_score_:.4f}")
