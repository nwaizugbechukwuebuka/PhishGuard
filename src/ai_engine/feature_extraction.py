...
class EmailFeatureExtractor:
    ...
    def __init__(self):
        ...
        self.suspicious_keywords = [
            ...
            'limited time', 'expire', 'confirm', 'update', 'winner', 'congratulations',
            'prize', 'lottery', 'free', 'bonus', 'offer', 'discount', 'deal',
            'security alert', 'account locked', 'login', 'password', 'credit card',
    ...
    async def extract_text_features(self, email_data: Dict[str, Any]) -> Dict[str, float]:
        """
        ...
        features['digit_ratio'] = sum(1 for c in content if c.isdigit()) / len(content) if content else 0
        features['special_char_ratio'] = sum(1 for c in content if not c.isalnum() and not c.isspace()) / len(content) if content else 0
        features['exclamation_count'] = content.count('!')
        features['question_count'] = content.count('?')
        
        # Suspicious keyword analysis
        features['suspicious_keyword_count'] = sum(1 for keyword in self.suspicious_keywords if keyword in combined_text)
        features['suspicious_keyword_ratio'] = features['suspicious_keyword_count'] / len(combined_text.split()) if combined_text.split() else 0
        
        # Urgency indicators
        urgency_words = ['urgent', 'immediate', 'asap', 'quickly', 'hurry', 'rush', 'emergency']
        features['urgency_score'] = sum(1 for word in urgency_words if word in combined_text)
    ...
    async def extract_url_features(self, email_data: Dict[str, Any]) -> Dict[str, float]:
        """
    ...
    async def extract_metadata_features(self, email_data: Dict[str, Any]) -> Dict[str, float]:
        """
        ...
        features['sender_special_chars'] = sum(1 for c in sender if not c.isalnum() and c not in '@.-_')
        
        # Domain analysis
        if '@' in sender:
            domain = sender.split('@')[1]
            features['sender_domain_length'] = len(domain)
            features['sender_legitimate_domain'] = 1 if domain in self.legitimate_domains else 0
    ...
    async def extract_behavioral_features(self, email_data: Dict[str, Any]) -> Dict[str, float]:
        """
        ...
        authority_words = ['bank', 'security', 'admin', 'support', 'official', 'government']
        features['authority_appeal'] = sum(1 for word in authority_words if word in combined_text)
        
        urgency_phrases = ['act now', 'limited time', 'expires soon', 'immediate action']
        features['urgency_phrases'] = sum(1 for phrase in urgency_phrases if phrase in combined_text)
        ...
        features['reward_appeal'] = sum(1 for word in reward_words if word in combined_text)
        
        # Action requests
        action_words = ['click', 'download', 'install', 'verify', 'confirm', 'update']
        features['action_requests'] = sum(1 for word in action_words if word in combined_text)
        
        # Personal information requests
        personal_info = ['password', 'ssn', 'social security', 'credit card', 'account number']
        features['personal_info_requests'] = sum(1 for info in personal_info if info in combined_text)
        
        # Spelling and grammar errors (simple heuristic)
        features['potential_typos'] = len(re.findall(r'\b\w*[0-9]+\w*\b', combined_text))
        ...
        features['money_mentions'] = sum(len(re.findall(pattern, combined_text)) for pattern in money_patterns)
    ...
    def _get_default_features(self) -> Dict[str, float]:
        ...
        return {
            # Text features
            'text_length': 0, 'subject_length': 0, 'word_count': 0,
            'sentence_count': 0, 'avg_word_length': 0, 'uppercase_ratio': 0,
            'digit_ratio': 0, 'special_char_ratio': 0, 'exclamation_count': 0,
            'question_count': 0, 'suspicious_keyword_count': 0,
            'suspicious_keyword_ratio': 0, 'urgency_score': 0,
            'has_html': 0, 'html_tag_count': 0,
            ...
            # URL features
            'url_count': 0, 'has_urls': 0,
            **self._get_default_url_features(),
            
            # Metadata features
            'sender_length': 0, 'sender_has_numbers': 0, 'sender_special_chars': 0,
            'sender_domain_length': 0, 'sender_legitimate_domain': 0,
            'domain_has_numbers': 0, 'domain_dash_count': 0,
            'domain_subdomain_count': 0, 'hour_of_day': 12,
            'day_of_week': 0, 'is_weekend': 0, 'is_business_hours': 1,
            
            # Behavioral features
            'authority_appeal': 0, 'urgency_phrases': 0, 'fear_appeal': 0,
            'reward_appeal': 0, 'action_requests': 0,
            'personal_info_requests': 0, 'potential_typos': 0,
            'repeated_punctuation': 0, 'money_mentions': 0,
        }
    
    def _get_default_url_features(self) -> Dict[str, float]:
        """Get default URL feature values."""
        return {
            'suspicious_domain_ratio': 0, 'ip_address_ratio': 0,
            'suspicious_tld_ratio': 0, 'shortened_url_ratio': 0,
            'long_url_ratio': 0,
        }
