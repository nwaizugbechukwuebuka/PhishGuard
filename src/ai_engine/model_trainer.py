...
class PhishingModelTrainer:
    ...
    def generate_synthetic_training_data(self, n_samples: int = 10000) -> Tuple[pd.DataFrame, np.ndarray]:
        ...
        phishing_patterns = {
            "urgent_keywords": ["urgent", "immediate", "expires", "act now", "limited time", "verify account"],
            "suspicious_phrases": ["click here", "update payment", "suspended account", "verify identity"],
            "credential_requests": ["enter password", "login required", "confirm details", "update billing"],
            "financial_terms": ["bank", "paypal", "credit card", "billing", "payment", "refund"],
            "authority_claims": ["security team", "IT department", "administrator", "official"],
            "fear_tactics": ["account closed", "legal action", "fraud detected", "security breach"]
        }
        
        legitimate_patterns = {
            "business_terms": ["quarterly report", "meeting", "project update", "team sync", "deadline"],
            "casual_communication": ["how are you", "catch up", "thanks", "please find attached", "best regards"],
            "informational": ["newsletter", "announcement", "update", "information", "notice"]
        }
        ...
        logger.info(f"Generated dataset with {len(df)} samples, {sum(labels)} phishing, {len(labels) - sum(labels)} legitimate")
        
        return df, labels
    
    def _generate_phishing_sample(self, patterns: Dict[str, List[str]]) -> Dict[str, Any]:
        ...
        sample = {
            ...
            "unusual_time": np.random.choice([0, 1], p=[0.4, 0.6]),  # Often unusual timing
    ...
    def _generate_legitimate_sample(self, patterns: Dict[str, List[str]]) -> Dict[str, Any]:
    ...
    def prepare_features(self, df: pd.DataFrame) -> Tuple[np.ndarray, List[str]]:
        ...
        # Feature names
        feature_names = numerical_features + [f"tfidf_{i}" for i in range(text_features.shape[1])]
    ...
    def train_models(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        for model_name, model in models.items():
            ...
            logger.info(f"{model_name} - Test Score: {test_score:.4f}, AUC: {auc_score:.4f}")
        
        # Select best model based on AUC score
        best_model_name = max(model_results.keys(), key=lambda k: model_results[k]['auc_score'])
        self.best_model = model_results[best_model_name]['model']
        
        logger.info(f"Best model: {best_model_name} with AUC: {model_results[best_model_name]['auc_score']:.4f}")
    ...
    def calculate_feature_importance(self):
        ...
        if self.best_model is None:
            logger.error("No trained model available for feature importance calculation")
            return
