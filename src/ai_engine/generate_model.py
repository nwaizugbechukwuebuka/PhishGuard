...
def create_phishing_classifier():
    ...
    text_content, numerical_features, labels = prepare_features_and_labels(training_data)
    ...
    model_pipeline = {
        ...
        'feature_names': [
            'sender_reputation', 'has_attachments', 'url_count',
            'suspicious_urls', 'urgency_score', 'grammar_errors'
        ],
        'model_version': '1.0.0',
...
def load_and_test_model(filepath):
    ...
    try:
        ...
        # Prepare features
        text_features = model_pipeline['vectorizer'].transform([test_email['content']]).toarray()
        ...
        logger.info(f"Test prediction: {'Phishing' if prediction == 1 else 'Legitimate'}")
