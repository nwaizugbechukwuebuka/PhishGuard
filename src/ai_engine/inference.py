...
class PhishingDetector:
    ...
    async def load_model(self) -> bool:
        ...
        try:
            ...
            logger.info(f"🤖 Loading phishing detection model from {self.model_path}...")
            ...
            self.feature_extractor = model_package.get('feature_extractor', EmailFeatureExtractor())
            self.model_metadata = {
            ...
            self.is_loaded = True
            logger.info(f"✅ Model loaded successfully: {self.model_metadata['model_name']}")
    ...
    async def predict_phishing(self, email_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"🔍 Phishing detection complete - Risk Score: {result['risk_score']:.1f}%")
            return result
    ...
    async def batch_predict(self, email_batch: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            if not self.is_loaded:
                await self.load_model()
                if not self.is_loaded:
                    return [self._get_error_prediction("Model not available") for _ in email_batch]
            ...
            # Process results
            results = []
            for i, (email_data, prediction, proba) in enumerate(zip(email_batch, predictions, predictions_proba)):
            ...
            logger.info(f"✅ Batch processing complete - {len(results)} emails analyzed")
            return results
    ...
    async def _explain_prediction(self, features: Dict[str, Any], confidence: float) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if features.get('has_html', 0) > 0 and features.get('html_tag_count', 0) > 10:
...
async def batch_predict_phishing(email_batch: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
