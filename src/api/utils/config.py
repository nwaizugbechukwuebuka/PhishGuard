...
class Settings(BaseSettings):
    ...
    JWT_SECRET_KEY: str = Field(default="change-this-jwt-secret-in-production", description="JWT secret key")
    ...
    GMAIL_CREDENTIALS_FILE: Optional[str] = Field(default=None, description="Gmail API credentials file path")
    GMAIL_TOKEN_FILE: Optional[str] = Field(default=None, description="Gmail API token file path")
    ...
    SLACK_CHANNEL: str = Field(default="#security-alerts", description="Default Slack channel")
    ...
    ML_CONFIDENCE_THRESHOLD: float = Field(default=0.75, description="ML confidence threshold")
    ML_BATCH_SIZE: int = Field(default=32, description="ML batch processing size")
    ML_MAX_EMAIL_LENGTH: int = Field(default=10000, description="Maximum email length for processing")
    
    # File Storage
    QUARANTINE_STORAGE_PATH: str = Field(
        default="src/quarantine_storage",
        description="Quarantine storage directory"
    )
    ...
    LOG_STORAGE_PATH: str = Field(
        default="src/quarantine_storage/logs",
    ...
    PROMETHEUS_METRICS_PORT: int = Field(default=8001, description="Prometheus metrics server port")
    ...
    SOAR_PLAYBOOK_ID: Optional[str] = Field(default=None, description="SOAR playbook ID")
    ...
    COMPLIANCE_REPORTING_ENABLED: bool = Field(default=True, description="Enable compliance reporting")
    ...
    DEV_RELOAD: bool = Field(default=False, description="Enable auto-reload in development")
    DEV_MOCK_AI: bool = Field(default=False, description="Mock AI responses in development")
    DEV_MOCK_INTEGRATIONS: bool = Field(default=False, description="Mock integrations in development")
    ...
    FEATURE_REAL_TIME_SCANNING: bool = Field(default=True, description="Enable real-time scanning")
    FEATURE_AUTOMATED_RESPONSE: bool = Field(default=True, description="Enable automated response")
    FEATURE_THREAT_INTELLIGENCE: bool = Field(default=True, description="Enable threat intelligence")
...
def validate_configuration() -> List[str]:
    ...
    if settings.SECRET_KEY == "change-this-secret-key-in-production" and settings.is_production():
        issues.append("SECRET_KEY must be changed in production")
    
    if settings.JWT_SECRET_KEY == "change-this-jwt-secret-in-production" and settings.is_production():
