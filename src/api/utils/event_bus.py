...
class EventBus:
    ...
    def subscribe(
        self, event_type: str, handler: Callable, priority: bool = False
    ) -> str:
        """
...
async def emit_threat_detected(
    threat_data: Dict[str, Any], source: str = "detection_engine"
):
...
async def emit_user_action(action_type: str, user_id: str, details: Dict[str, Any]):
    ...
    return await event_bus.emit(
        f"user_{action_type}",
        {"user_id": user_id, "action": action_type, "details": details},
        source="user_service",
    )
...
async def emit_system_event(
    ...
    """Emit system event."""
    event_bus = get_event_bus()
    return await event_bus.emit(event_type, details, source="system", priority=priority)
