...
class ModelExplainer:
    ...
    def _generate_lime_explanation(
        self, text: str, model, vectorizer, prediction_score: float
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            explanation = self.lime_explainer.explain_instance(
                text, predict_proba_fn, num_features=10, top_labels=1
            )
            ...
            lime_data["top_features"] = sorted(
                lime_data["top_features"], key=lambda x: x["importance"], reverse=True
            )
    ...
    def _extract_feature_importance(
        self, text: str, model, vectorizer, feature_names: Optional[List[str]]
    ) -> Dict[str, Any]:
        """Extract global feature importance from the model."""
        try:
            importance_data = {"global_importance": [], "text_specific_importance": []}
                        ...
                        importance_data["text_specific_importance"].append(
                            {
                                ...
                                "score": float(score),
                                "frequency": (
                                    int(score) if score.is_integer() else float(score)
                                ),
                            }
                        )
    ...
    def _analyze_suspicious_patterns(
        self, email_content: str, subject: str, sender: str
    ) -> Dict[str, Any]:
        ...
        try:
                    ...
                    patterns["urgency_indicators"].append(
                        {
                            "indicator": word,
                            "location": (
                                "subject" if word in subject_lower else "content"
                            ),
                    ...
                    patterns["phishing_keywords"].append(
                        {
                            ...
                            "suspicion_level": (
                                "high"
                                if keyword in ["password", "login", "verify"]
                                else "medium"
                            ),
                        }
                    )
            ...
            if sender and "@" in sender:
                ...
                suspicious_domains = [
                    "gmail.com",
                    "yahoo.com",
                    "hotmail.com",  # Common free domains used in phishing
                ]
    ...
    def _extract_context(
        self, text: str, keyword: str, context_length: int = 50
    ) -> str:
    ...
    def _generate_recommendations(
        self, prediction_score: float, suspicious_patterns: Dict[str, Any]
    ) -> List[str]:
        ...
        if suspicious_patterns["urgency_indicators"]:
            recommendations.append(
                "Be wary of urgency tactics - legitimate organizations rarely require immediate action"
            )

        if suspicious_patterns["phishing_keywords"]:
            recommendations.append(
                "Be cautious of requests for verification or account updates"
            )

        if suspicious_patterns["social_engineering"]:
            recommendations.append(
                "Be skeptical of emotional manipulation or pressure tactics"
            )
