...
APP_INFO = Info(
    "phishguard_app_info", "PhishGuard application information", registry=REGISTRY
)
...
ACTIVE_CONNECTIONS = Gauge(
    "phishguard_active_connections", "Number of active connections", registry=REGISTRY
)
...
ACTIVE_SESSIONS = Gauge(
    "phishguard_active_sessions", "Number of active user sessions", registry=REGISTRY
)
...
MEMORY_USAGE_BYTES = Gauge(
    "phishguard_memory_usage_bytes", "Memory usage in bytes", registry=REGISTRY
)

CPU_USAGE_PERCENT = Gauge(
    "phishguard_cpu_usage_percent", "CPU usage percentage", registry=REGISTRY
)

DISK_USAGE_BYTES = Gauge(
    "phishguard_disk_usage_bytes", "Disk usage in bytes", ["path"], registry=REGISTRY
)
...
class MetricsCollector:
    ...
    def record_email_processing(
        ...
        try:
            EMAILS_PROCESSED_TOTAL.labels(platform=platform, status=status).inc()

            EMAIL_PROCESSING_DURATION.labels(
                platform=platform, processing_stage=processing_stage
            ).observe(duration)

            if is_phishing and threat_level:
                EMAILS_DETECTED_PHISHING.labels(
                    platform=platform, threat_level=threat_level
                ).inc()
    ...
    def record_model_prediction(
        ...
        try:
            ...
            if confidence is not None:
                MODEL_CONFIDENCE_SCORE.labels(model_type=model_type).observe(confidence)
        except Exception as e:
    ...
    def record_integration_request(
        self, integration: str, operation: str, status: str, duration: float
    ):
    ...
    def record_quarantine_action(self, action: str, platform: str):
        ...
        try:
            QUARANTINE_ACTIONS_TOTAL.labels(action=action, platform=platform).inc()
        except Exception as e:
    ...
    def record_alert_sent(self, alert_type: str, channel: str, status: str):
        ...
        try:
            ALERTS_SENT_TOTAL.labels(
                type=alert_type, channel=channel, status=status
            ).inc()
        except Exception as e:
...
def monitor_function(
    metric_name: str = None, labels: Dict[str, str] = None, duration_metric: bool = True
):
