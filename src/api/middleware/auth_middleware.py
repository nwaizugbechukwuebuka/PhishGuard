...
class AuthMiddleware:
    ...
    async def __call__(self, request: Request, call_next):
        ...
        try:
            ...
            logger.info(
                f"Authenticated user: {user_info.get('email')} for {request.method} {request.url.path}"
            )
...
async def verify_user_permissions(
    required_permissions: list, current_user: User = Depends(get_current_user)
) -> bool:
    """
    ...
    )  # This would be implemented on User model
...
class RoleBasedAccessControl:
    ...
    def require_roles(allowed_roles: list):
        ...
        def role_checker(current_user: User = Depends(get_current_user)):
            ...
            ]  # Assuming User has roles relationship
...
limit_login_attempts = rate_limiter.limit_requests(
    max_requests=5, window_seconds=300
)  # 5 attempts per 5 minutes
limit_api_requests = rate_limiter.limit_requests(
