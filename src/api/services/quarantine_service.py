...
class QuarantineService:
    ...
    async def quarantine_email(
        ...
        try:
            ...
            existing = self.db.query(QuarantinedEmail).filter(
            ...
            await self.event_bus.emit("email_quarantined", {
    ...
    async def release_email(
        ...
        try:
            ...
            quarantined_email = self.db.query(QuarantinedEmail).filter(
                QuarantinedEmail.id == quarantine_id
            ).first()
            ...
            # Emit release event
            await self.event_bus.emit("email_released", {
    ...
    async def delete_quarantined_email(
        ...
        try:
            ...
            # Emit deletion event
            await self.event_bus.emit("email_deleted", {
                "quarantine_id": str(quarantine_id),
                "permanent": permanent,
                "reason": deletion_reason
            })
    ...
    async def extend_quarantine(
        ...
        try:
            ...
            quarantined_email.extension_count = (quarantined_email.extension_count or 0) + 1
    ...
    async def get_quarantined_emails(
        ...
        try:
            ...
            if quarantine_reason:
                query = query.filter(QuarantinedEmail.quarantine_reason == quarantine_reason)
            
            if sender_email:
                query = query.filter(QuarantinedEmail.sender_email.ilike(f"%{sender_email}%"))
            
            if recipient_email:
                query = query.filter(QuarantinedEmail.recipient_email.ilike(f"%{recipient_email}%"))
            ...
            quarantined_emails = query.order_by(
                desc(QuarantinedEmail.quarantined_at)
    ...
    async def get_quarantine_statistics(
        ...
        try:
            ...
            threat_levels = self.db.query(
                ...
                func.count(QuarantinedEmail.id)
            ).filter(
                query.whereclause
            ).group_by(QuarantinedEmail.threat_level).all()
            
            threat_level_dist = {level.value: count for level, count in threat_levels}
            
            # Quarantine reason distribution
            quarantine_reasons = self.db.query(
                QuarantinedEmail.quarantine_reason,
                func.count(QuarantinedEmail.id)
            ).filter(
                query.whereclause
            ).group_by(QuarantinedEmail.quarantine_reason).all()
            ...
            avg_confidence = self.db.query(
                func.avg(QuarantinedEmail.confidence_score)
            ).filter(query.whereclause).scalar() or 0.0
            ...
            return {
                ...
                "release_rate": (released_count / total_quarantined * 100) if total_quarantined > 0 else 0,
    ...
    async def analyze_threat_trends(
        ...
        try:
            ...
            threat_trends = self.db.query(
                ...
                'date', QuarantinedEmail.threat_level
            ).order_by('date').all()
            ...
            confidence_trends = self.db.query(
                date_trunc.label('date'),
        ...
        try:
            ...
            if attachments:
                ...
                for i, attachment in enumerate(attachments):
                    att_file = attachments_dir / f"attachment_{i}_{attachment.get('filename', 'unknown')}"
    ...
    async def _deliver_released_email(self, quarantined_email: QuarantinedEmail):
        ...
        try:
            ...
            await self.event_bus.emit("deliver_released_email", {
                "quarantine_id": str(quarantined_email.id),
                "recipient": quarantined_email.recipient_email,
                "sender": quarantined_email.sender_email,
