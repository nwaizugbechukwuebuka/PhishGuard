...
from ..models.simulation import ParticipantStatus, SimulationParticipant
from ..models.user import Role, User, UserSecuritySettings, UserSettings
...
class UserService:
    ...
    async def delete_user(
        self, user_id: uuid.UUID, deleted_by: Optional[uuid.UUID] = None
    ) -> bool:
        """
        Soft delete a user.
    ...
    async def get_user_details(self, user_id: uuid.UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            security_settings = (
                self.db.query(UserSecuritySettings)
            ...
            recent_activities = await self._get_user_recent_activities(
                user_id, limit=20
            )
            ...
            return {
                ...
                "security_settings": {
                    "mfa_enabled": (
                        security_settings.mfa_enabled if security_settings else False
                    ),
                    "login_notifications": (
                        security_settings.login_notifications
                        if security_settings
                        else True
                    ),
                    "security_alerts": (
                        security_settings.security_alerts if security_settings else True
                    ),
                    "session_timeout": (
                        security_settings.session_timeout if security_settings else 60
                    ),
                    "allowed_ip_ranges": (
                        security_settings.allowed_ip_ranges if security_settings else []
                    ),
                },
                "notification_settings": {
                    "email_notifications": (
                        user_settings.email_notifications if user_settings else True
                    ),
                    "sms_notifications": (
                        user_settings.sms_notifications if user_settings else False
                    ),
                    "notification_frequency": (
                        user_settings.notification_frequency
                        if user_settings
                        else "immediate"
                    ),
        ...
        try:
            ...
            if simulation_participants:
                ...
                clicked_simulations = len(
                    [
                        p
                        for p in simulation_participants
                        if p.link_clicked_at is not None
                    ]
                )
                reported_simulations = len(
                    [p for p in simulation_participants if p.reported_at is not None]
                )

                click_rate = (
                    clicked_simulations / total_simulations
                    if total_simulations > 0
                    else 0
                )
                report_rate = (
                    reported_simulations / total_simulations
                    if total_simulations > 0
                    else 0
                )
                ...
                risk_score -= (
                    report_rate * 2.0
                )  # Subtract up to 2 points for high report rate
                ...
                recent_simulations = [
                    ...
                    for p in simulation_participants
                    if p.email_sent_at
                    and p.email_sent_at > datetime.utcnow() - timedelta(days=90)
                ]

                if recent_simulations:
                    recent_clicked = len(
                        [p for p in recent_simulations if p.link_clicked_at is not None]
                    )
                    recent_click_rate = recent_clicked / len(recent_simulations)
                    risk_score += (
                        recent_click_rate * 2.0
                    )  # Additional weight for recent behavior
    ...
    async def get_user_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            mfa_enabled_count = (
                self.db.query(UserSecuritySettings)
                .filter(UserSecuritySettings.mfa_enabled == True)
                .count()
            )
            mfa_adoption_rate = (
    ...
    async def bulk_activate_users(
        self, user_ids: List[uuid.UUID], updated_by: Optional[uuid.UUID] = None
    ) -> Dict[str, Any]:
        """Bulk activate users."""
        try:
    ...
    async def update_user_security_settings(
        self, user_id: uuid.UUID, **settings
    ) -> bool:
    ...
    async def update_user_settings(self, user_id: uuid.UUID, **settings) -> bool:
        ...
        try:
            ...
            if not user_settings:
                user_settings = UserSettings(id=uuid.uuid4(), user_id=user_id)
    ...
    async def _get_user_simulation_stats(self, user_id: uuid.UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            participants = (
                self.db.query(SimulationParticipant)
                .filter(SimulationParticipant.user_email == user.email)
                .all()
            )
            ...
            click_rate = (
                (links_clicked / total_simulations * 100)
                if total_simulations > 0
                else 0
            )
            report_rate = (
                (reported_phishing / total_simulations * 100)
                if total_simulations > 0
                else 0
            )
            ...
            last_simulation = max(
                [p.email_sent_at for p in participants if p.email_sent_at], default=None
            )
            ...
            return {
                ...
                "last_simulation_date": (
                    last_simulation.isoformat() if last_simulation else None
                ),
            }
    ...
    async def _get_user_login_history(
        ...
        try:
            ...
            login_logs = (
                ...
                .filter(
                    and_(
                        AuditLog.user_id == user_id, AuditLog.action == ActionType.LOGIN
                    )
                )
    ...
    async def _get_user_recent_activities(
    ...
    async def log_action(
        ...
        await self._log_user_action(
            action, resource_id, user_id, {**details, "resource_type": resource_type}
        )
