...
from ..models.simulation import SimulationCampaign, SimulationTemplate, SimulationResult
...
class SimulationService:
    ...
    async def create_campaign(
        ...
        try:
            ...
            # Validate templates
            templates = self.db.query(SimulationTemplate).filter(
                SimulationTemplate.id.in_(template_ids)
            ).all()
            ...
            target_users = await self._resolve_target_users(target_user_ids, target_groups)
            ...
            campaign = SimulationCampaign(
                ...
                status=CampaignStatus.SCHEDULED.value if scheduled_date else CampaignStatus.DRAFT.value,
    ...
    async def launch_campaign(
        ...
        try:
            # Get campaign
            campaign = self.db.query(SimulationCampaign).filter(
            ...
            if campaign.status not in [CampaignStatus.DRAFT.value, CampaignStatus.SCHEDULED.value]:
                raise ValueError(f"Campaign cannot be launched in status: {campaign.status}")
            ...
            templates = self.db.query(SimulationTemplate).filter(
                SimulationTemplate.id.in_(campaign.template_ids)
            ).all()
            
            results = self.db.query(SimulationResult).filter(
                SimulationResult.campaign_id == campaign_id
            ).all()
            ...
            for result in results:
                try:
                    ...
                    success = await self._send_simulation_email(result, template, campaign)
                    if success:
                        emails_sent += 1
                        result.email_sent_at = datetime.utcnow()
                except Exception as e:
                    logger.error(f"Error sending simulation email to user {result.user_id}: {str(e)}")
            ...
            logger.info(f"Simulation campaign launched: {campaign.name} - {emails_sent}/{len(results)} emails sent")
            return launch_data
    ...
    async def record_user_action(
        ...
        try:
            ...
            if action == UserAction.CLICKED:
                ...
                result.time_to_click = (action_time - result.email_sent_at).total_seconds() if result.email_sent_at else None
            
            elif action == UserAction.REPORTED:
                result.reported = True
                result.reported_at = action_time
                result.time_to_report = (action_time - result.email_sent_at).total_seconds() if result.email_sent_at else None
            ...
            campaign = self.db.query(SimulationCampaign).filter(
                SimulationCampaign.id == result.campaign_id
            ).first()
            
            # Trigger training if user failed and training is enabled
            if campaign and campaign.training_enabled and self._user_failed_simulation(result):
            ...
            logger.info(f"User action recorded: {action.value} by user {result.user_id}")
            return action_data
    ...
    async def get_campaign_results(
        ...
        try:
            ...
            susceptibility_rate = ((clicked_count + downloaded_count + credentials_count) / max(emails_sent, 1)) * 100
            ...
            medium_risk_users = len([r for r in results if 30 <= (r.risk_score or 0) <= 70])
            ...
            campaign_results = {
                ...
                "risk_assessment": {
                    ...
                    "overall_risk_score": await self._calculate_campaign_risk_score(results)
                },
                "training_metrics": await self._get_training_metrics(campaign_id) if campaign.training_enabled else None
            }
            
            # Add detailed user results if requested
            if include_details:
                campaign_results["user_results"] = await self._format_user_results(results)
    ...
    async def get_user_simulation_history(
        ...
        try:
            ...
            current_risk_level = sum(recent_risk_scores) / len(recent_risk_scores) if recent_risk_scores else 0
            ...
            history_data = {
                ...
                "simulation_summary": {
                    ...
                    "susceptibility_rate": ((clicked_count + downloaded_count + credentials_count) / max(total_simulations, 1)) * 100,
                ...
                "detailed_history": [
                    {
                        "campaign_id": str(result.campaign_id),
                        "simulation_date": result.email_sent_at.isoformat() if result.email_sent_at else None,
                ...
                "recommendations": await self._generate_user_recommendations(user_id, results)
            }
        ...
        try:
            ...
            for user_id in user_ids:
                try:
                    ...
                    logger.warning(f"Error creating training notification for user {user_id}: {str(e)}")
            ...
            logger.info(f"Training scheduled: {training_type} for {len(user_ids)} users")
    ...
    async def get_simulation_analytics(
        ...
        try:
            ...
            campaigns = self.db.query(SimulationCampaign).filter(
                and_(
            ...
            results = self.db.query(SimulationResult).filter(
                SimulationResult.campaign_id.in_(campaign_ids)
            ...
            analytics = {
                ...
                "overall_performance": await self._calculate_overall_performance(results),
                "trend_analysis": await self._analyze_simulation_trends(campaigns, results),
                "template_effectiveness": await self._analyze_template_effectiveness(campaigns, results),
                "user_behavior_patterns": await self._analyze_user_behavior_patterns(results),
                ...
                "recommendations": await self._generate_analytics_recommendations(campaigns, results)
            }
    ...
    async def _resolve_target_users(
        ...
        try:
            ...
            # Add specific users
            if user_ids:
                specific_users = self.db.query(User).filter(
            ...
            if groups:
                group_users = self.db.query(User).filter(
                    and_(User.department.in_(groups), User.is_active == True)
                ).all()
        ...
        try:
            for user in target_users:
                result = SimulationResult(
                    ...
                    user_id=user.id,
                    template_id=random.choice(templates).id,  # Random template assignment
                    clicked=False,
                    reported=False,
    ...
    async def _send_simulation_email(
        ...
        try:
            ...
            subject = template.subject.replace("{user_name}", f"{user.first_name} {user.last_name}")
            body = template.body.replace("{user_name}", f"{user.first_name} {user.last_name}")
    ...
    def _user_failed_simulation(self, result: SimulationResult) -> bool:
        """Check if user failed the simulation."""
        return (result.clicked or result.downloaded_attachment or 
                result.entered_credentials) and not result.reported
    ...
    async def _calculate_user_risk_score(self, result: SimulationResult) -> float:
    ...
    async def _get_training_metrics(self, campaign_id: uuid.UUID) -> Dict[str, Any]:
        ...
        training_triggered = len([r for r in results if self._user_failed_simulation(r)])
    ...
    async def _format_user_results(self, results: List[SimulationResult]) -> List[Dict[str, Any]]:
        ...
        for result in results:
            ...
            formatted_results.append({
                ...
                "email_sent": result.email_sent_at.isoformat() if result.email_sent_at else None,
                "clicked": result.clicked,
                "reported": result.reported,
                "downloaded_attachment": result.downloaded_attachment,
                "entered_credentials": result.entered_credentials,
                "risk_score": result.risk_score,
                "training_completed": result.training_completed,
                "last_action": result.last_action_at.isoformat() if result.last_action_at else None
            })
        
        return formatted_results
    
    async def _calculate_campaign_risk_score(self, results: List[SimulationResult]) -> float:
    ...
    async def _calculate_user_improvement_trend(self, results: List[SimulationResult]) -> str:
        ...
        recent_failures = len([r for r in recent_results if self._user_failed_simulation(r)])
        older_failures = len([r for r in older_results if self._user_failed_simulation(r)])
    ...
    async def _generate_user_recommendations(
        ...
        if click_count > len(recent_results) * 0.5:
            recommendations.append("Focus on identifying phishing indicators before clicking links")
        
        if report_count < len(recent_results) * 0.3:
            recommendations.append("Practice reporting suspicious emails promptly")
        
        avg_risk = sum([r.risk_score or 0 for r in recent_results]) / len(recent_results)
        if avg_risk > 60:
            recommendations.append("Enroll in advanced security awareness training")
        
        if not recommendations:
            recommendations.append("Continue current security practices and stay vigilant")
        
        return recommendations
    
    async def _calculate_overall_performance(self, results: List[SimulationResult]) -> Dict[str, Any]:
    ...
    async def _analyze_simulation_trends(
        self,
        campaigns: List[SimulationCampaign],
        results: List[SimulationResult]
    ) -> Dict[str, Any]:
        ...
        for month, month_results in monthly_data.items():
            click_rate = (len([r for r in month_results if r.clicked]) / len(month_results)) * 100
            report_rate = (len([r for r in month_results if r.reported]) / len(month_results)) * 100
    ...
    async def _analyze_template_effectiveness(
        ...
        template_performance = defaultdict(lambda: {"clicks": 0, "reports": 0, "total": 0})
    ...
    async def _analyze_user_behavior_patterns(self, results: List[SimulationResult]) -> Dict[str, Any]:
        """Analyze user behavior patterns."""
        patterns = {
            "quick_clickers": len([r for r in results if r.time_to_click and r.time_to_click < 60]),
            "careful_reporters": len([r for r in results if r.time_to_report and r.time_to_report > 300]),
            "repeat_offenders": 0,  # Would need more complex logic
            "security_champions": len([r for r in results if r.reported and not r.clicked])
        }
        
        return patterns
    
    async def _analyze_training_impact(self, results: List[SimulationResult]) -> Dict[str, Any]:
        ...
        trained_failure_rate = len([r for r in trained_users if self._user_failed_simulation(r)]) / len(trained_users)
        untrained_failure_rate = len([r for r in untrained_users if self._user_failed_simulation(r)]) / len(untrained_users)
        
        return {
            "trained_failure_rate": trained_failure_rate * 100,
            "untrained_failure_rate": untrained_failure_rate * 100,
            "improvement_factor": (untrained_failure_rate - trained_failure_rate) / untrained_failure_rate * 100 if untrained_failure_rate > 0 else 0
        }
    
    async def _analyze_risk_distribution(self, results: List[SimulationResult]) -> Dict[str, Any]:
    ...
    async def _generate_analytics_recommendations(
        ...
        if results:
            ...
            if click_rate > 30:
                recommendations.append("Increase frequency of phishing awareness training")
            
            if report_rate < 20:
                recommendations.append("Improve reporting mechanisms and user education")
            
            if len(campaigns) < 4:
                recommendations.append("Conduct more frequent simulation campaigns")
        
        if not recommendations:
            recommendations.append("Continue current simulation practices and monitor trends")
