...
class NotificationService:
    ...
    async def create_notification(
        ...
        try:
            ...
            if template_id:
                ...
                if template:
                    title = (
                        template.subject_template.format(**metadata)
                        if metadata
                        else template.subject_template
                    )
                    message = (
                        template.body_template.format(**metadata)
                        if metadata
                        else template.body_template
                    )
        ...
        try:
            ...
            for recipient_id in recipient_ids:
                try:
                    ...
                    logger.error(
                        f"Error creating notification for user {recipient_id}: {str(e)}"
                    )
                    continue
            ...
            logger.info(
                f"Bulk notifications created: {len(notifications)} of {len(recipient_ids)}"
            )
    ...
    async def get_notification_statistics(
        ...
        try:
            ...
            # Delivery rate
            delivery_rate = (
                (delivered_count / total_notifications * 100)
            ...
            # Read rate
            read_rate = (
                (read_count / delivered_count * 100) if delivered_count > 0 else 0
            )
    ...
    async def update_user_notification_preferences(
        ...
        try:
            ...
            if not user_prefs:
                user_prefs = NotificationPreference(id=uuid.uuid4(), user_id=user_id)
    ...
    async def get_user_notification_preferences(
        ...
        try:
            ...
            return {
                ...
                "quiet_hours_start": (
                    prefs.quiet_hours_start.isoformat()
        ...
        try:
            ...
            for notification in pending_notifications:
                try:
                    ...
                    logger.error(
                        f"Error delivering notification {notification.id}: {str(e)}"
                    )
                    continue
    ...
    async def _deliver_email(self, notification: Notification) -> Dict[str, Any]:
        ...
        try:
            # Get recipient user
            recipient = (
                self.db.query(User).filter(User.id == notification.recipient_id).first()
            )
    ...
    async def _get_user_preferred_channels(
        ...
        try:
            ...
            if notification_type in [
                ...
                NotificationType.IMMEDIATE_THREAT,
            ]:
                channels = [
