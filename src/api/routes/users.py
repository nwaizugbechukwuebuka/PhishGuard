...
from ..utils.security import generate_temporary_password, verify_password
...
async def create_user(
    ...
    try:
        ...
        if user_request.send_welcome_email:
            background_tasks.add_task(send_welcome_email, user.id, temp_password, db)
...
async def update_current_user_security_settings(
    ...
    try:
        ...
        await user_service.update_user_security_settings(
            user_id=current_user.id, **security_settings.dict(exclude_unset=True)
        )
        ...
        await user_service.log_action(
            ...
            details={
                "updated_settings": list(
                    security_settings.dict(exclude_unset=True).keys()
                )
            },
        )
...
async def update_current_user_settings(
    ...
    try:
        ...
        await user_service.update_user_settings(
            user_id=current_user.id, **user_settings.dict(exclude_unset=True)
        )
    ...
    try:
        ...
        if operation_request.operation == "activate":
            result = await user_service.bulk_activate_users(
                user_ids=operation_request.user_ids, updated_by=current_user.id
            )
            ...
            if (
                not operation_request.parameters
                or "role" not in operation_request.parameters
            ):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
            ...
            if (
                not operation_request.parameters
                or "department" not in operation_request.parameters
            ):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid bulk operation"
            )
...
async def reset_user_password(
    ...
    try:
        ...
        await user_service.reset_user_password(
            user_id=user_id, temporary_password=temp_password, reset_by=current_user.id
        )

        # Send password reset email
        background_tasks.add_task(send_password_reset_email, user_id, temp_password, db)
        ...
        await user_service.log_action(
            ...
            resource_id=user_id,
            user_id=current_user.id,
            details={"action": "password_reset", "user_email": user.email},
        )
...
async def send_password_reset_email(
    user_id: uuid.UUID, temporary_password: str, db: Session
):
