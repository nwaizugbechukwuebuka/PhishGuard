...
from ..models.quarantine import QuarantinedEmail, QuarantineAttachment, QuarantineReason, QuarantineAction, ThreatLevel, QuarantineStatus
...
async def quarantine_email(
    ...
    try:
        ...
        background_tasks.add_task(
            notify_quarantine_action,
            db,
            quarantined_email.id,
            "quarantined"
        )
...
async def list_quarantined_emails(
    ...
    reason: Optional[QuarantineReason] = Query(None, description="Filter by quarantine reason"),
    ...
    try:
        ...
        quarantined_emails = query.order_by(desc(QuarantinedEmail.quarantined_at))\
        ...
        email_responses = [
            QuarantineEmailResponse(**email.to_dict())
...
async def get_quarantined_email(
    ...
    """
    try:
        quarantined_email = db.query(QuarantinedEmail).filter(
            QuarantinedEmail.id == email_id
        ).first()
...
async def get_quarantine_stats(
    ...
    try:
        ...
        active_quarantines = sum(1 for e in quarantined_emails if e.status == QuarantineStatus.ACTIVE)
        expired_quarantines = sum(1 for e in quarantined_emails if e.is_expired())
        released_emails = sum(1 for e in quarantined_emails if e.status == QuarantineStatus.RELEASED)
        deleted_emails = sum(1 for e in quarantined_emails if e.status == QuarantineStatus.DELETED)
        ...
        reason_breakdown = {}
        for email in quarantined_emails:
            reason = email.quarantine_reason.value if email.quarantine_reason else "unknown"
        ...
        for i in range(8):
            ...
            weekly_trends.append({
                ...
                "threat_levels": {
                    level.value: sum(1 for e in week_emails if e.threat_level == level)
