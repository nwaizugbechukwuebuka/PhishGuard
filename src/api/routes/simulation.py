...
async def get_simulation_templates(
    simulation_type: Optional[SimulationType] = Query(
...
async def create_simulation_campaign(
    ...
    try:
        ...
        template = await simulation_service.get_template_by_id(
        ...
        if campaign_request.start_date <= datetime.utcnow():
            background_tasks.add_task(start_simulation_campaign, campaign.id, db)
        else:
...
async def get_simulation_campaigns(
    ...
    try:
        ...
        campaigns = await simulation_service.get_campaigns(
            status=status, template_id=template_id, skip=skip, limit=limit
        )
        ...
        for campaign in campaigns:
            ...
            result.append(
                SimulationCampaignResponse(
                    ...
                    created_by=(
                        campaign.created_by_user.email
...
@router.get(
    ...
    response_model=List[SimulationParticipantResponse],
)
async def get_campaign_participants(
    campaign_id: uuid.UUID,
    status: Optional[ParticipantStatus] = Query(
        None, description="Filter by participant status"
    ),
    department: Optional[str] = Query(None, description="Filter by department"),
    skip: int = Query(0, ge=0, description="Number of participants to skip"),
    limit: int = Query(
        100, ge=1, le=100, description="Number of participants to return"
    ),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
...
@router.get(
    "/campaigns/{campaign_id}/analytics", response_model=SimulationAnalyticsResponse
)
async def get_campaign_analytics(
    ...
    try:
        ...
        analytics = await simulation_service.generate_campaign_analytics(
            campaign_id=campaign_id, include_detailed=include_detailed
        )
...
async def schedule_simulation_campaign(
    campaign_id: uuid.UUID, start_date: datetime, db: Session
):
    ...
    try:
        ...
        logger.info(
            f"Simulation campaign {campaign_id} scheduled to start at {start_date}"
        )
    except Exception as e:
...
async def send_simulation_emails(
    campaign_id: uuid.UUID, participant_ids: List[uuid.UUID], db: Session
):
    ...
    try:
        ...
        logger.error(
            f"Error sending simulation emails for campaign {campaign_id}: {str(e)}"
        )


async def schedule_simulation_emails(
    campaign_id: uuid.UUID, timing: Dict[str, datetime], db: Session
):
    ...
    try:
        ...
        logger.error(
            f"Error scheduling simulation emails for campaign {campaign_id}: {str(e)}"
        )
...
async def send_campaign_emails(campaign_id: uuid.UUID, db: Session):
    ...
    try:
        ...
        logger.error(
            f"Error sending campaign emails for campaign {campaign_id}: {str(e)}"
        )
