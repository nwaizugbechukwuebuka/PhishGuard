...
from ..utils.security import generate_reset_token, get_password_hash, verify_password
    ...
    try:
        ...
        if not verify_password(request.password, user.password_hash):
            ...
            if user.failed_login_attempts >= 5:
                ...
                background_tasks.add_task(
                    send_account_lockout_notification, user.email, client_ip
                )
...
async def setup_mfa(
    ...
    try:
        ...
        totp_uri = pyotp.totp.TOTP(secret).provisioning_uri(
            name=current_user.email, issuer_name="PhishGuard"
        )
...
async def verify_mfa(
    ...
    try:
        ...
        background_tasks.add_task(send_mfa_enabled_notification, current_user.email)
...
async def disable_mfa(
    ...
    try:
        ...
        background_tasks.add_task(send_mfa_disabled_notification, current_user.email)
...
async def log_security_event(
    ...
    try:
        audit_log = AuditLog.create_audit_entry(
            ...
            status=(
                StatusType.SUCCESS
                if action in [ActionType.LOGIN, ActionType.PASSWORD_CHANGE]
                else StatusType.FAILURE
            ),
        )
