...
from ..models.quarantine import ThreatLevel
...
async def get_compliance_report(
    framework: Optional[str] = Query(
        None, description="Compliance framework (GDPR, HIPAA, SOX, etc.)"
    ),
    start_date: Optional[datetime] = Query(
        None, description="Start date for compliance period"
    ),
    end_date: Optional[datetime] = Query(
        None, description="End date for compliance period"
    ),
    current_user: User = Depends(get_current_admin_user),
    db: Session = Depends(get_db),
):
...
async def get_executive_summary(
    period: str = Query(
        "monthly", description="Summary period: weekly, monthly, quarterly"
    ),
    ...
    try:
        ...
        summary = await report_service.generate_executive_summary(
            start_date=start_date, end_date=date, period=period
        )
    ...
    try:
        ...
        if request.format == "json":
            ...
            background_tasks.add_task(
                generate_custom_report_file, request, current_user.id, db
            )
    ...
    try:
        ...
        if report_type == "threat-analytics":
            report_data = await report_service.generate_threat_analytics(
                start_date=start_date, end_date=end_date, **filter_dict
            )
        ...
        if format == "csv":
            content, media_type, filename = generate_csv_report(
                report_data, report_type
            )
        elif format == "pdf":
            content, media_type, filename = generate_pdf_report(
                report_data, report_type
            )
        elif format == "excel":
            content, media_type, filename = generate_excel_report(
                report_data, report_type
            )
        else:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid export format"
            )
    ...
    try:
        templates = [
            ...
            },
            {
                ...
                "available_filters": ["framework", "violation_type", "severity"],
            },
            {
                "id": "user-activity",
            ...
            },
            {
                ...
                "available_filters": ["time_period", "department", "priority_level"],
            },
        ]
...
def generate_pdf_report(data: Dict[str, Any], report_type: str) -> tuple:
    ...
    p.drawString(
        50, 730, f"Generated: {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')}"
    )
...
def generate_excel_report(data: Dict[str, Any], report_type: str) -> tuple:
    ...
    content = buffer.getvalue()
    filename = (
        f"{report_type}_report_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.xlsx"
    )
...
async def generate_custom_report_file(
    request: CustomReportRequest, user_id: uuid.UUID, db: Session
):
