...
async def create_notification(
    ...
    try:
        ...
        background_tasks.add_task(
            notification_service.schedule_delivery, notification.id
        )
...
async def create_bulk_notifications(
    ...
    try:
                ...
                if not "@" in recipient:  # Assume it's a user ID
                    ...
                    if user:
                        ...
                        failed_recipients.append(
                            {"recipient": recipient, "error": "User not found"}
                        )
                        continue
        ...
        if not request.send_individually and created_notifications:
            background_tasks.add_task(
                notification_service.schedule_bulk_delivery, created_notifications
            )
...
async def list_notifications(
    ...
    channel: Optional[NotificationChannel] = Query(
        None, description="Filter by delivery channel"
    ),
    ...
    start_date: Optional[datetime] = Query(
        None, description="Filter notifications from this date"
    ),
    end_date: Optional[datetime] = Query(
        None, description="Filter notifications until this date"
    ),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
...
async def get_notification(
    ...
    try:
        ...
        if not current_user.is_admin:
            if (
                notification.recipient_id != current_user.id
                and notification.recipient_email != current_user.email
            ):
                raise HTTPException(
        ...
        if (
            notification.recipient_id == current_user.id
            or notification.recipient_email == current_user.email
        ):
...
async def get_notification_stats(
    ...
    try:
        ...
        total_delivered = sum(
            1 for n in notifications if n.status == DeliveryStatus.DELIVERED
        )
        total_failed = sum(
            1 for n in notifications if n.status == DeliveryStatus.FAILED
        )
        total_pending = sum(
            1 for n in notifications if n.status == DeliveryStatus.PENDING
        )
        ...
        for notification in notifications:
            notification_type = (
                notification.type.value if notification.type else "unknown"
            )
        ...
        return NotificationStatsResponse(
            ...
            average_delivery_time=(
                round(average_delivery_time, 2) if average_delivery_time else None
            ),
