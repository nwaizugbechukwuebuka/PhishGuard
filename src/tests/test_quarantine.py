...
class TestQuarantineService:
    ...
    def test_quarantine_search_and_filter(self):
        ...
        with patch.object(self.quarantine_service, 'get_db') as mock_get_db:
            ...
            mock_db.query().filter().filter().filter().filter().all.return_value = mock_results
...
class TestQuarantineFileManagement:
    ...
    def test_quarantine_storage_cleanup(self):
        ...
        old_email_path = self.quarantine_service.store_email_files(
            email_id=1, 
            email_content={'subject': 'Old Email'}
        )
        
        recent_email_path = self.quarantine_service.store_email_files(
            email_id=2, 
            email_content={'subject': 'Recent Email'}
        )
    ...
    def test_quarantine_storage_security(self):
        ...
        assert any('passwd' in str(f) for f in files)  # Filename preserved but sanitized
...
class TestQuarantineCompliance:
    ...
    def test_audit_log_creation(self, mock_get_db):
        ...
        action_data = {
            ...
            'metadata': {
                'confidence': 0.95,
                'threat_indicators': ['suspicious_url']
            }
        }
    ...
    def test_data_retention_policy(self):
        ...
        with patch.object(self.quarantine_service, 'get_db') as mock_get_db:
            ...
            cutoff_date = datetime.now() - timedelta(days=retention_policy['quarantine_retention_days'])
            mock_old_items = [Mock(id=1), Mock(id=2)]
            mock_db.query().filter().all.return_value = mock_old_items
            
            cleanup_results = self.quarantine_service.enforce_retention_policy(retention_policy)
    ...
    def test_quarantine_access_control(self):
        ...
        admin_permissions = {
            'user_id': 2,
            'role': 'admin',
            'permissions': ['view_all_quarantine', 'release_any_quarantine', 'delete_quarantine']
        }
        
        # Test user permissions
        assert self.quarantine_service.check_permission(
...
class TestQuarantineIntegration:
    ...
    def test_quarantine_notification_integration(self, mock_notification):
        ...
        with patch.object(self.quarantine_service, 'quarantine_email') as mock_quarantine:
            mock_quarantine.return_value = {'success': True, 'quarantine_id': 123}
            
            result = self.quarantine_service.quarantine_with_notification(quarantine_data)
