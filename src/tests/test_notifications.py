...
class TestNotificationService:
    ...
    def test_get_user_notifications(self, mock_get_db):
        ...
        mock_db.query().filter().order_by().limit().all.return_value = mock_notifications
    ...
    def test_delete_notification(self, mock_get_db):
        ...
        result = self.notification_service.delete_notification(notification_id=1, user_id=1)
    ...
    def test_notification_priority_handling(self):
        ...
        with patch.object(self.notification_service, 'create_notification') as mock_create:
...
class TestNotificationTasks:
    ...
    def test_send_daily_digest_task(self, mock_mail, mock_db):
        ...
        mock_db_session.query().filter().filter().count.side_effect = [25, 23]  # Threats detected/blocked
...
class TestNotificationPreferences:
    ...
    def test_user_notification_preferences(self):
        ...
        with patch.object(notification_service, 'update_preferences') as mock_update:
            ...
            result = notification_service.set_user_preferences(user_id=1, preferences=preferences)
    ...
    def test_notification_filtering_by_preferences(self):
        ...
        notifications = [
            {'threat_type': 'phishing', 'confidence': 0.95, 'timestamp': datetime.now().replace(hour=10)},
            {'threat_type': 'spam', 'confidence': 0.75, 'timestamp': datetime.now().replace(hour=14)},
            {'threat_type': 'malware', 'confidence': 0.65, 'timestamp': datetime.now().replace(hour=22)},
            {'threat_type': 'phishing', 'confidence': 0.85, 'timestamp': datetime.now().replace(hour=16)}
        ]
        
        notification_service = NotificationService()
        filtered = notification_service.filter_by_preferences(notifications, user_preferences)
