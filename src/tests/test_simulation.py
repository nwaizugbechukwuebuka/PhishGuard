...
from api.models.simulation import SimulationCampaign, SimulationEmail, SimulationInteraction
...
class TestSimulationService:
    ...
    def test_user_simulation_history(self):
        ...
        with patch.object(self.simulation_service, 'get_db') as mock_get_db:
            ...
            history = self.simulation_service.get_user_history(user_email='user@company.com')
...
class TestSimulationTemplates:
    ...
    def test_it_support_template_generation(self):
        ...
        assert 'IT Support' in template['subject'] or 'Technical Support' in template['subject']
    ...
    def test_social_engineering_template(self):
        ...
        assert ('urgent' in template['body'].lower() or 
                'confidential' in template['body'].lower())
    ...
    def test_template_difficulty_levels(self):
        ...
        for difficulty in difficulties:
            ...
            if difficulty == 'easy':
                ...
                assert any(indicator in template['body'].lower() 
                          for indicator in ['click here immediately', 'urgent action required'])
    ...
    def test_template_localization(self):
        ...
        assert any(word in template['body'].lower() 
...
class TestSimulationInteractions:
    ...
    def test_credential_submission_tracking(self):
        ...
        with patch.object(self.simulation_service, 'track_interaction') as mock_track:
            ...
            result = self.simulation_service.process_credential_submission(submission_data)
    ...
    def test_interaction_analytics(self):
        ...
        with patch.object(self.simulation_service, 'get_db') as mock_get_db:
            ...
            mock_interactions = [
                ...
                Mock(interaction_type='link_clicked', timestamp=datetime.now()),
                Mock(interaction_type='credentials_entered', timestamp=datetime.now()),
                Mock(interaction_type='reported_suspicious', timestamp=datetime.now() + timedelta(minutes=5))
            ]
            mock_db.query().join().filter().all.return_value = mock_interactions
            
            analytics = self.simulation_service.calculate_interaction_analytics(campaign_id)
...
class TestSimulationEducation:
    ...
    def test_educational_feedback_generation(self):
        ...
        interaction_context = {
            ...
            'difficulty_level': 'medium',
            'user_history': {
                'previous_simulations': 3,
                'pass_rate': 0.67
            }
        }
        
        feedback = self.simulation_service.generate_educational_feedback(interaction_context)
    ...
    def test_personalized_training_recommendations(self):
        ...
        recommendations = self.simulation_service.generate_training_recommendations(user_profile)
...
class TestSimulationCampaignManagement:
    ...
    def test_campaign_scheduling_optimization(self):
        ...
        target_users = [
            {'email': f'user{i}@company.com', 'timezone': 'UTC-5', 'department': 'Finance'}
            for i in range(100)
        ]
        ...
        schedule = self.simulation_service.optimize_email_schedule(target_users, schedule_config)
    ...
    def test_campaign_template_ab_testing(self):
        ...
        assert abs(len(assignment['variant_A']) - len(assignment['variant_B'])) <= 1  # Even split
    ...
    def test_campaign_compliance_controls(self):
        ...
        compliance_check = self.simulation_service.check_campaign_compliance(campaign_data)
        ...
        frequency_check = self.simulation_service.check_simulation_frequency(frequent_campaign)
...
class TestSimulationReporting:
    ...
    def test_executive_dashboard_report(self):
        ...
        with patch.object(self.simulation_service, 'get_db') as mock_get_db:
            ...
            mock_db.query().join().filter().count.side_effect = [500, 300, 50, 150]  # sent, opened, clicked, reported
    ...
    def test_trend_analysis(self):
        ...
        time_periods = [
            {'start': datetime.now() - timedelta(days=180), 'end': datetime.now() - timedelta(days=90)},
        ...
        with patch.object(self.simulation_service, 'get_db') as mock_get_db:
            ...
            mock_db.query().join().filter().filter().count.side_effect = [
                200, 120, 30, 50,  # Period 1: sent, opened, clicked, reported
                250, 180, 25, 80   # Period 2: sent, opened, clicked, reported
            ]
            ...
            assert trends['improvement_metrics']['click_rate_change'] < 0  # Improved (lower click rate)
            assert trends['improvement_metrics']['report_rate_change'] > 0  # Improved (higher report rate)
